 ...
 * @var $query \common\models\RobotDdk
 */
$this->title = '多多客采集';
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'value' => function ($model) {
                            return \common\models\RobotDdk::FROM_CATEGORY[$model->from_cid];
                        }
                    ],
                    [
                        'attribute' => 'to_cid',
                    ...
                    [
                        ...
                        'buttons' => [
                            'run' => function($url, $model, $key) {
                                return '<a href="javascript:void(0);" onclick="runRobots(' . $model->from_cid . ')">采集</a>';
                            }
                        ]
                    ],
                ]
...
<script>
    function runRobots(from_cid) {
        var run = function (pageNum, num) {
            $.post('<?= Url::to(['robot-ddk/run'])?>', {pageNum: pageNum, num: num, opt_id: from_cid}, function (response) {
                if (response.code === 0) {
