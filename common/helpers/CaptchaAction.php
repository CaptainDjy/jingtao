...
class CaptchaAction extends Action
{
    ...
    /**
     * @var int the font size. Defaults to null.
     */
    ...
    public function run()
    {
        if (Yii::$app->request->getQueryParam(self::REFRESH_GET_VAR) !== null) {
            ...
            if (Yii::$app->request->getQueryParam(self::REFRESH_GET_VAR) == 'app') {
                $data = [
                    'code' => 0,
                    'data' => 'data:image/png;base64,' . chunk_split(base64_encode($this->renderImage($code))),
                    'msg' => '请求成功'
                ];
                return $data;
            }
    ...
    protected function renderImageByGD($code)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            $fontSize = $this->fontSize ? $this->fontSize : (int)(mt_rand(26, 32) * $scale * 0.8);
            $angle = mt_rand(-10, 10);
            $letter = $code[$i];
